...
export default function StructuredDataInPage(
  url,
  tag_title,
  meta_description,
  updatedAt,
  ...
  return {
    ...
    "@graph": [
      ...
      getBreadcrumbs(url, page, pillarData).length > 0 ? {
        "@type": "BreadcrumbList",
        // "@id": `${url}#breadcrumb`, 
        "itemListElement": getBreadcrumbs(url, page, pillarData)
      } : "",
...
function getBreadcrumbs(url, page, pillarData) {
  ...
  let breadcrumbsTemplate = generateBreadcrumbJson(breadcrumbs, page, pillarData);

  return breadcrumbsTemplate;
}

//! Generate template from breadcrumbs
function generateBreadcrumbJson(breadcrumbs, page, pillarData) {
  ...
  for (let i = 1; i < breadcrumbs.length; i++) {
    ...
    if (i === 1) {
      breadcrumbItem = {
        "@type": "ListItem",
        "position": i + 1,
        "name": (page && page.nome) ? (pillarData) ? pillarData?.pillar?.data?.attributes?.nome : page?.nome : breadcrumbs[i].replace('-', ' '),
      ...
      breadcrumbItem = {
        "@type": "ListItem",
        "position": i + 1,
        "name": (page && page.nome) ? page?.nome : (page && page.titolo) ? page?.titolo : breadcrumbs[i].replace('-', ' '),
...
//       "item": {
//         "@type": "WebPage",
//         "id": `${baseUrl}${breadcrumbs[i - 1]}/${breadcrumbs[i]}`
//       }
//     };
//   }
// }