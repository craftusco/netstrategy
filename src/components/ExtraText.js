...
export default function ExtraText({mt, data, title}) {
  ...
  //* Set active chapter on scroll
  // useEffect(() => {
  //   if(data.length === 1) return;
  //   let ctx = gsap.context(() => {
  //     chapters.current.forEach((chap, i) => {
        ...
        <ContentWrapper>
          <TitleSection>
            <Title>
              ...
              <IndexList>
                {data.map((link, i) => (
                  <li key={i} className={activeChap === i ? 'active' : ''}><span className="dot">●</span> <a onClick={(e) => nextChapt(e, link.titolo, i)} href={'#'+toSlugText(link.titolo)}>{link.titolo}</a></li>
                ))}
              </IndexList>
            </Title>
          </TitleSection>
