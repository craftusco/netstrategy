    ...
    <Wrapper className="thank-you" blackVersion={blackVersion}>
      ...
      <HeroThankYou ref={heroThankYouRef}>
        <Text blackVersion={blackVersion}>
            ...
            <p>{thankYou.sottotitolo_1}</p> */}
             {/* <h3>{thankYou.titolo_outro}</h3> */}
            <p>Ti confermiamo l'avvenuta disiscrizione. <br></br> Ci dispiace che tu sia andato via.</p>
          <a
            onClick={(e) => {
              goToSection(e, "#we-are-curious");
            }}
            href="#we-are-curious"
          >
            <svg
      ...
      <Container>
        <PageHeading id="we-are-curious">
          <Title blackVersion={blackVersion}>
            <h2>{thankYou.titolo_2}</h2>
            <a
              onClick={(e) => {
                goToSection(e, "#feedback");
              }}
              href="#feedback"
            >
              <svg
          ...
          <Image>
            <div className="image-wrapper">
              <img src={getPath(thankYou.media_1.data.attributes.url)} alt={thankYou?.media_1?.data?.attributes?.alternativeText ? thankYou?.media_1?.data?.attributes?.alternativeText : "Immagine persone in ufficio"}></img>
            </div>
          </Image>
        </PageHeading>
        <FeebackWrapper>
          <Questionnaire blackVersion={blackVersion} data={thankYou.questionnaire}/>
          <ImageWrapper>
            <div className="img">
              <img src={getPath(thankYou.media_2.data.attributes.url)} alt={thankYou?.media_2?.data?.attributes?.alternativeText ? thankYou?.media_2?.data?.attributes?.alternativeText : "Immagine persone in ufficio"}></img>
            </div>
          </ImageWrapper>
        </FeebackWrapper>
      </Container>
      <PostCardWrapper>
        <PostCard blackVersion={blackVersion}>
          <h3>{thankYou.titolo_outro}</h3>
          <p>{thankYou.paragrafo_outro}</p>
        </PostCard>
      </PostCardWrapper>
      <ImageSlider
        data={thankYou.media_slider}
        mt="clamp(2rem, calc(1.03rem + 4.14vw), 6rem)"
      />
      <Divider
        mt="clamp(2rem, calc(1.03rem + 4.14vw), 6rem)"
        isWhite={blackVersion}
      />
      {/* <Heading
        isWhite={blackVersion}
        data={{ titolo: "projects." }}
