...
export async function getStaticProps({ params }) {
  const urls = [
    ...
    {
      name: "articles",
      // url: `https://www.netstrategy.it/api/journal-details?populate=deep,5&filters[utente][slug]=${params["dettaglio-autore"]}`,
      url: `https://www.netstrategy.it/api/journal-details?populate=deep,5&filters[utente][slug]=${params["dettaglio-autore"]}&pagination[limit]=4&sort=original_date:desc`,
      transform: false,
    },
    {
      name: "blog",
...
export default function authorDetail({ data, staticData }) {
  ...
  const articles = data.articles.map(({ attributes }, i) => {
    return {
      ...attributes,
      get counter() {
        return parseInt(i) + i;
      ...
      get date_1() {
        return `${moment(attributes.original_date).format("MMM")}/N. ${(
          parseInt(i) + 1
        )
          .toString()
          .padStart(2, "0")}`;
      ...
      get date_2() {
        return moment(attributes.original_date).format("DD.MM.YYYY");
      },
    };
  });
  //! REFS
    ...
    <div ref={containerRef}>
      ...
      <List className="blog-list-ref">
        {articles.length > 0 ? (
          ...
          <CenteredDiv>Nessun articolo presente</CenteredDiv>
        )}
      </List>
      {/* Buttom */}
      <ButtonContainer>
        <RedLink
          img={getPath(data.blog.attributes.hero.immagine.data.attributes.url)}
          link="/blog"
        >
          <PrimaryButton>Leggi di più</PrimaryButton>
        </RedLink>
      </ButtonContainer>
      {/* Footer */}
